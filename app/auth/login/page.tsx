...
import { Phone, ArrowRight, AlertCircle, FileVideo, Megaphone, ArrowLeft } from 'lucide-react';
...
function LoginContent() {
    ...
    const defaultRedirect = role === 'creator' ? '/dashboard' : '/advertiser';

    const handleGoogleLogin = async () => {
        setIsLoading(true);
        setError(null);
        const supabase = createClient();
        const next = redirect || defaultRedirect;

        const { error } = await supabase.auth.signInWithOAuth({
            provider: 'google',
            options: {
                redirectTo: `${window.location.origin}/auth/callback?next=${next}&role=${role}`,
            },
        });
            ...
            <div className="w-full max-w-md relative z-10">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-[var(--vp-text-secondary)] text-sm mt-2">
                        {step === 'role'
                            ...
                            : `Sign in as ${role === 'creator' ? 'Creator' : 'Advertiser'}`
                        }
                    </p>
                </div>
                ...
                {step === 'role' && (
                    <div className="space-y-4">
                        <button
                            onClick={() => { setRole('creator'); setStep('login'); }}
                            className="w-full glass-card-static p-6 text-left transition-all hover:ring-2 hover:ring-[var(--vp-purple)] hover:ring-opacity-50 group"
                        >
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-xl flex items-center justify-center"
                                    style={{ background: 'linear-gradient(135deg, var(--vp-purple), var(--vp-green))' }}>
                                    <FileVideo size={24} className="text-white" />
                                </div>
                                <div className="flex-1">
                                    <p className="text-lg font-semibold group-hover:text-[var(--vp-purple-light)] transition">
                                        Creator Login
                                    </p>
                                    <p className="text-sm text-[var(--vp-text-muted)]">
                                        Share content, earn money from views
                                    </p>
                                </div>
                                <ArrowRight size={20} className="text-[var(--vp-text-muted)] group-hover:text-[var(--vp-purple-light)] transition" />
                            </div>
                        </button>

                        <button
                            onClick={() => { setRole('advertiser'); setStep('login'); }}
                            className="w-full glass-card-static p-6 text-left transition-all hover:ring-2 hover:ring-[var(--vp-cyan)] hover:ring-opacity-50 group"
                        >
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-xl flex items-center justify-center"
                                    style={{ background: 'linear-gradient(135deg, var(--vp-cyan), var(--vp-purple))' }}>
                                    <Megaphone size={24} className="text-white" />
                                </div>
                                <div className="flex-1">
                                    <p className="text-lg font-semibold group-hover:text-[var(--vp-cyan)] transition">
                                        Advertiser Login
                                    </p>
                                    <p className="text-sm text-[var(--vp-text-muted)]">
                                        Create campaigns, reach micro-influencers
                                    </p>
                                </div>
                                <ArrowRight size={20} className="text-[var(--vp-text-muted)] group-hover:text-[var(--vp-cyan)] transition" />
                            </div>
                        </button>
                    </div>
                )}
                ...
                {(step === 'login' || step === 'otp') && (
                    <div className="glass-card-static p-8">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 text-sm text-[var(--vp-text-muted)] hover:text-white transition mb-6"
                        >
                            <ArrowLeft size={14} />
                            Change role
                        </button>
